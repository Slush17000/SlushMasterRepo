...
async function initialize() {
    ...
    if (mesh.texCoords) {
        attributes.addAttribute('texPosition', mesh.vertexCount, 2, mesh.texCoords);
    }
...
function updateCameraWithKeyboard(camera, elapsed) {
    ...
    if (keys['w'])
        camera.advance(elapsed * moveSpeedLeft);
    if (keys['s'])
        camera.advance(-elapsed * moveSpeedLeft);
    if (keys['a'])
        camera.strafe(-elapsed * moveSpeedLeft);
    if (keys['d'])
        camera.strafe(elapsed * moveSpeedLeft);
...
function renderScene(camera, clipFromEye, side) {
...
function renderPlayer(playerCamera, viewCamera, clipFromEye, playerVao, color) {
    ...
    const modelPosition = new Vector3(playerCamera.from.x + forward.x * modelDistance, playerCamera.from.y + 1, playerCamera.from.z + forward.z * modelDistance);
    ...
    const worldFromPlayer = Matrix4.translate(modelPosition.x, modelPosition.y, modelPosition.z).multiplyMatrix(Matrix4.rotateY(yawDegrees - 90))
    ...
    // Use solid color for player models (not texture)

...
let canvas;
let texturedShaderProgram;
let wireframeShaderProgram;
let texturedVao;
let wireframeVao;
let texture;
let trackball;
...
function screenToWorld(x, y) {
    ...
    const mousePixel = new Vector4(x - panelWidth, // Adjust for right panel offset
    canvas.height - 1 - y, 0, 1);
    const mouseNormalized = new Vector4(mousePixel.x / panelWidth * 2 - 1, mousePixel.y / canvas.height * 2 - 1, 0.5, 1);
...
function worldToTex(worldX, worldY) {
    return {
...
function setLightingUniforms(program, ambient, specular, shininess) {

...
let lightPosition = new Vector3(0, 0, -5);
...
function updateLightPosition(elapsed) {
    // Update z-coordinate based on keyboard input
    const zSpeed = 0.005; // units per millisecond
    if (keys['w'])
        lightZ -= elapsed * zSpeed; // Move closer (negative z)
    if (keys['s'])
        lightZ += elapsed * zSpeed; // Move farther (positive z)
}

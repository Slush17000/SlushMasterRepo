...
function updateRotation(elapsed) {
    ...
    if (!gamepadInput) {
        ...
        if (recentInstance) {
            ...
            if (keys['w'])
                recentInstance.translateZ -= elapsed * translateSpeed; // Forward
            if (keys['s'])
                recentInstance.translateZ += elapsed * translateSpeed; // Backward
            if (keys['a'])
                recentInstance.translateX -= elapsed * translateSpeed; // Left
            if (keys['d'])
                recentInstance.translateX += elapsed * translateSpeed; // Right
            if (keys['q'])
                recentInstance.rotationY -= elapsed * rotationSpeed; // Rotate left
            if (keys['e'])
                recentInstance.rotationY += elapsed * rotationSpeed; // Rotate right
            if (keys['r'])
                recentInstance.translateY += elapsed * translateSpeed; // Up
            if (keys['f'])
                recentInstance.translateY -= elapsed * translateSpeed; // Down
            // Update the world transformation matrix for this instance
            recentInstance.worldFromPose = Matrix4.translate(recentInstance.translateX, recentInstance.translateY, recentInstance.translateZ)
                .multiplyMatrix(Matrix4.rotateY(recentInstance.rotationY * 180.0 / Math.PI));
        }
            ...
            // If no instances exist, control the original model (fallback behavior)
            if (keys['w'])
                rotationX -= elapsed * rotationSpeed; // Rotate up
            if (keys['s'])
                rotationX += elapsed * rotationSpeed; // Rotate down
            if (keys['a'])
                rotationY -= elapsed * rotationSpeed; // Rotate left
            if (keys['d'])
                rotationY += elapsed * rotationSpeed; // Rotate right
        }
    }
}

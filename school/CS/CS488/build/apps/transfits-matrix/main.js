...
let canvas;
let shaderProgram;
let vao1, vao2, vao3, vao4, vao5, vao6, vaoFill;
let attributes, attributes1, attributes2, attributes3, attributes4, attributes5, attributes6;
let vectorGroups = [];
let clipFromWorld;
let worldFromModel = new Float32Array(Matrix4.identity().elements);
...
function render() {
    ...
    shaderProgram.setUniformMatrix4fv('worldFromModel', worldFromModel);
    vaoFill.drawIndexed(gl.TRIANGLES);
    // #2
    shaderProgram.setUniform3f('rgb', 0.0, 1.0, 1.0);
    transformation = transformation.multiplyMatrix(Matrix4.translate(6.0, 0.0, 0.0));
    transformation = transformation.multiplyMatrix(Matrix4.rotateZ(-30));
    worldFromModel = new Float32Array(transformation.elements);
    transformation = Matrix4.identity();
    shaderProgram.setUniformMatrix4fv('worldFromModel', worldFromModel);
    vaoFill.drawIndexed(gl.TRIANGLES);
    // #3
    shaderProgram.setUniform3f('rgb', 1.0, 0.0, 1.0);
    transformation = transformation.multiplyMatrix(Matrix4.translate(12.0, 0.0, 0.0));
    transformation = transformation.multiplyMatrix(Matrix4.scale(1.0, -1.5, 1.0));
    worldFromModel = new Float32Array(transformation.elements);
    transformation = Matrix4.identity();
    shaderProgram.setUniformMatrix4fv('worldFromModel', worldFromModel);
    vaoFill.drawIndexed(gl.TRIANGLES);
    ...
    transformation = transformation.multiplyMatrix(Matrix4.translate(0.0, 6.0, 0.0));
    transformation = transformation.multiplyMatrix(Matrix4.scale(1.5, -1.0, 1.0));
    transformation = transformation.multiplyMatrix(Matrix4.rotateZ(180));
    worldFromModel = new Float32Array(transformation.elements);
    transformation = Matrix4.identity();
    shaderProgram.setUniformMatrix4fv('worldFromModel', worldFromModel);
    vaoFill.drawIndexed(gl.TRIANGLES);
    ...
    transformation = transformation.multiplyMatrix(Matrix4.translate(6.0, 6.0, 0.0));
    transformation = transformation.multiplyMatrix(Matrix4.scale(-1.0, 0.5, 1.0));
    transformation = transformation.multiplyMatrix(Matrix4.rotateZ(90));
    worldFromModel = new Float32Array(transformation.elements);
    transformation = Matrix4.identity();
    shaderProgram.setUniformMatrix4fv('worldFromModel', worldFromModel);
    vaoFill.drawIndexed(gl.TRIANGLES);
    ...
    transformation = transformation.multiplyMatrix(Matrix4.translate(12.0, 6.0, 0.0));
    transformation = transformation.multiplyMatrix(Matrix4.scale(1.0, 1.5, 1.0));
    transformation = transformation.multiplyMatrix(Matrix4.rotateZ(120));
    worldFromModel = new Float32Array(transformation.elements);
    transformation = Matrix4.identity();
    shaderProgram.setUniformMatrix4fv('worldFromModel', worldFromModel);
    vaoFill.drawIndexed(gl.TRIANGLES);
    vaoFill.unbind();
    worldFromModel = new Float32Array(Matrix4.identity().elements);
    shaderProgram.setUniformMatrix4fv('worldFromModel', worldFromModel);

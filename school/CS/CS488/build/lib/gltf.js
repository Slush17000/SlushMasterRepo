...
const getTransform = (keyFrames, duration) => {
    ...
    switch (frames.previous.type) {
        case 'translation':
        case 'scale': {
            const result = frames.previous.transform.lerp(frames.next.transform, progression);
            return result;
        }
        case 'rotation': {
            const result = frames.previous.transform.slerp(frames.next.transform, progression);
            return result;
        }
    }
};
const get = (c, elapsed) => {
    const t = c && c.translation.length > 0 ? getTransform(c.translation, elapsed) : new Vector3(0, 0, 0);
    const r = c && c.rotation.length > 0 ? getTransform(c.rotation, elapsed) : new Quaternion(0, 0, 0, 1);
    const s = c && c.scale.length > 0 ? getTransform(c.scale, elapsed) : new Vector3(1, 1, 1);
    return { t, r, s };
};
...
function extractAnimation(gltf, animation, buffers) {
    ...
    channels.forEach((channel) => {
        ...
        for (let i = 0; i < channel.time.buffer.length; i++) {
            ...
            const transform = channel.type === 'rotation'
                ? new Quaternion(channel.buffer.buffer[i * size + offset], channel.buffer.buffer[i * size + offset + 1], channel.buffer.buffer[i * size + offset + 2], channel.buffer.buffer[i * size + offset + 3])
                : new Vector3(channel.buffer.buffer[i * size + offset], channel.buffer.buffer[i * size + offset + 1], channel.buffer.buffer[i * size + offset + 2]);
...
export class Model {
    ...
    animationTransforms(blendTime = 0) {
        ...
        for (let rootAnimation of this.activeAnimations) {
            ...
            for (let key of Object.keys(this.animations[rootAnimation.key])) {
                ...
                for (let animation of this.activeAnimations) {
                    if (rootAnimation.key == animation.key || blend <= 0)
                        continue;
                ...
                const rotTransform = transform.r.toMatrix4();
    ...
    skinTransforms(blendTime, inverse = true) {
    ...
    applyTransform(appliedTransforms, transforms, parentTransform, skin, nodeIndex, inverse) {

...
export class Matrix4 {
    ...
    multiplyMatrix(that) {
        ...
        for (let r = 0; r < 4; ++r) {
            for (let c = 0; c < 4; ++c) {
                const dot = this.get(r, 0) * that.get(0, c) +
    ...
    multiplyPosition(v) {
        ...
        const w = 1;
        return new Vector3(m.get(0, 0) * x + m.get(0, 1) * y + m.get(0, 2) * z + m.get(0, 3) * w, m.get(1, 0) * x + m.get(1, 1) * y + m.get(1, 2) * z + m.get(1, 3) * w, m.get(2, 0) * x + m.get(2, 1) * y + m.get(2, 2) * z + m.get(2, 3) * w);
    }
    static look(from, forward, worldUp) {
    ...
    static rotateAround(axis, degrees) {
        const m = Matrix4.identity();
        const radians = degrees * Math.PI / 180;
    ...
    multiplyVector3(v) {
        const x = v.x, y = v.y, z = v.z;
        return new Vector3(this.get(0, 0) * x + this.get(0, 1) * y + this.get(0, 2) * z, this.get(1, 0) * x + this.get(1, 1) * y + this.get(1, 2) * z, this.get(2, 0) * x + this.get(2, 1) * y + this.get(2, 2) * z);
    }
    multiplyVector4(v) {
        return new Vector4(this.get(0, 0) * v.x + this.get(0, 1) * v.y + this.get(0, 2) * v.z + this.get(0, 3) * v.w, this.get(1, 0) * v.x + this.get(1, 1) * v.y + this.get(1, 2) * v.z + this.get(1, 3) * v.w, this.get(2, 0) * v.x + this.get(2, 1) * v.y + this.get(2, 2) * v.z + this.get(2, 3) * v.w, this.get(3, 0) * v.x + this.get(3, 1) * v.y + this.get(3, 2) * v.z + this.get(3, 3) * v.w);
    }
    inverse() {
        let m = new Matrix4();

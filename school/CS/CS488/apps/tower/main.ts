...
async function initialize() {
  ...
  // Load a glTF model
  const model = await Gltf.readFromUrl('../../models/tower.gltf');
  ...
  const toggle = document.getElementById('toggle-rotate') as HTMLButtonElement;
  if (toggle) {
    toggle.addEventListener('click', () => {
      isRotating = !isRotating;
      toggle.textContent = isRotating ? 'Stop Rotate' : 'Start Rotate';
      if (isRotating) startAnimation(); else stopAnimation();
    });
...
function render() {
  ...
  // Draw glTF model
  shaderProgram.setUniform3f('factors', 0.2, 0.2, 0.1);
  shaderProgram.setUniform3f('offsets', 0.0, -3.0, 0.0);

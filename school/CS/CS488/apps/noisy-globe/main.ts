...
function getTerrainColor(noiseValue: number): [number, number, number] {
...
let cachedNoiseData: { continentNoise: any, detailNoise: any, width: number, height: number } | null = null;

function renderGlobe() {
  if (!cachedNoiseData) return;
...
async function initialize() {
  ...
  const button = document.getElementById('planetToggle') as HTMLButtonElement;
  if (button) {

...
let vao: VertexArray;
let clipFromWorld: Matrix4;
let rotationMatrix: Matrix4;
let modelCentroid: Vector3;
let modelRadius: number;
let zoomFactor: number = 3;
...
async function loadAndDisplayModel(modelPath: string) {
...
async function loadModel(filename: string): Promise<Box[]> {
...
function calculateModelBounds(boxes: Box[]) {
    if (boxes.length === 0) {
...
function createBoxelMesh(boxes: Box[]): { positions: Float32Array; colors: Float32Array; indices: Uint32Array } {

...
interface AnimationConfig {
  ...
  returnToIdle: boolean; // Whether to return to idle after completion
}
...
class AnimationController {
  ...
  private animationConfigs: Map<AnimationState, AnimationConfig>;
  private model: gltf.Model;
  private previousVerticalVelocity: number;
  private wasJumping: boolean;

  constructor(model: gltf.Model) {
    this.model = model;
  ...
  private extractAnimationDurations() {
    ...
    for (const [animName, channels] of Object.entries(this.model.animations)) {
      ...
      for (const channel of Object.values(channels)) {
        for (const transformType of ['translation', 'rotation', 'scale']) {
          const keyframes = (channel as any)[transformType];
  ...
  public requestAnimation(state: AnimationState): boolean {
  ...
  public isAttacking(): boolean {
    ...
    const isAttackAnimation = (
      this.currentAnimation === AnimationState.RIGHT_PUNCH ||
  ...
  public setAnimationSpeed(state: AnimationState, multiplier: number) {
    const config = this.animationConfigs.get(state);
    if (config) {
  ...
  public updateJumpAnimation(camera: FirstPersonCamera) {
...
let modelLeft: gltf.Model;  // Separate model instance for left player
let modelRight: gltf.Model; // Separate model instance for right player
let canvas: HTMLCanvasElement;
let compassLeftDiv: HTMLDivElement;
let compassRightDiv: HTMLDivElement;
let altimeterLeftDiv: HTMLDivElement;
let altimeterRightDiv: HTMLDivElement;
let healthBarLeftDiv: HTMLDivElement;
let healthBarRightDiv: HTMLDivElement;
...
let gameState: GameState = GameState.WAITING_FOR_START;
let leftPlayerReady: boolean = false;
let rightPlayerReady: boolean = false;

// Color selection
let leftPlayerColorIndex: number = 5; // Default to Dark Blue
let rightPlayerColorIndex: number = 3; // Default to Green
let leftDPadUpPressed: boolean = false;
let leftDPadDownPressed: boolean = false;
let rightDPadUpPressed: boolean = false;
let rightDPadDownPressed: boolean = false;
let countdownValue: number = 5;
let countdownTimer: number = 0;
const countdownInterval: number = 1000; // 1 second per count

// UI elements for start/countdown messages
let leftStartMessageDiv: HTMLDivElement;
let rightStartMessageDiv: HTMLDivElement;
let leftColorSelectDiv: HTMLDivElement;
let rightColorSelectDiv: HTMLDivElement;
let leftCountdownDiv: HTMLDivElement;
let rightCountdownDiv: HTMLDivElement;

// Button press tracking for start button (button 9 = Start on Xbox controller)
let leftStartButtonPressed: boolean = false;
let rightStartButtonPressed: boolean = false;

// Stopwatch elements and timing
let leftStopwatchDiv: HTMLDivElement;
let rightStopwatchDiv: HTMLDivElement;
let stopwatchStartTime: number = 0;
let stopwatchRunning: boolean = false;

// Win condition tracking
let gameWon: boolean = false;
let winningPlayer: number = 0; // 1 for left, 2 for right
let winningTime: string = '';
let leftResultDiv: HTMLDivElement;
let rightResultDiv: HTMLDivElement;
let diamondVisible: boolean = true;

// Rematch tracking
let leftRematchDiv: HTMLDivElement;
let rightRematchDiv: HTMLDivElement;
let leftPlayerRematch: boolean = false;
let rightPlayerRematch: boolean = false;
let leftBackButtonPressed: boolean = false;
let rightBackButtonPressed: boolean = false;

// Health system
const MAX_HEALTH = 100;
let leftPlayerHealth: number = MAX_HEALTH;
let rightPlayerHealth: number = MAX_HEALTH;
let leftPlayerDead: boolean = false;
let rightPlayerDead: boolean = false;
let leftDeathScreenDiv: HTMLDivElement;
let rightDeathScreenDiv: HTMLDivElement;
const PUNCH_DAMAGE = 10;
const KICK_DAMAGE = 15;

// Track last hit to prevent multiple hits per attack
let leftPlayerLastHitTime: number = 0;
let rightPlayerLastHitTime: number = 0;
const HIT_COOLDOWN = 500; // milliseconds between hits

let shaderProgram: ShaderProgram;
let sunShaderProgram: ShaderProgram;
let skyboxShaderProgram: ShaderProgram;

let vaoGltf: VertexArray;
let sunVao: VertexArray;
let towerVao: VertexArray;
let wallVao: VertexArray;
let playerLeftVao: VertexArray;
let playerRightVao: VertexArray;
let skyboxVao: VertexArray;
let diamondVao: VertexArray;
let vao: VertexArray;
let skyboxTexture: WebGLTexture | null = null;

// Diamond position for end marker
let diamondPosition: Vector3;
let diamondRotation: number = 0;
...
let leftPlayerAnimController: AnimationController;
let rightPlayerAnimController: AnimationController;

let lightPosition = new Vector3(0, 0, 0);
let towerLightPositions: Vector3[] = [];
let towerLightBobOffset: number = 0; // For bobbing animation
let currentLightTime: number = 0; // Current time for animations
let terrainCenterX: number;
let terrainCenterZ: number;
let terrainMinX: number;
let terrainMaxX: number;
let terrainMinZ: number;
let terrainMaxZ: number;
let sunOrbitRadius: number;
let sunHeight: number;
let grassTexture: WebGLTexture | null = null;
let platformTexture: WebGLTexture | null = null;
...
let leftGamepadKonamiSequence: string[] = [];
let rightGamepadKonamiSequence: string[] = [];
let lastLeftGamepadKonamiInputTime = 0;
let lastRightGamepadKonamiInputTime = 0;

// Button state tracking for edge detection
const leftGamepadButtonStates: boolean[] = [];
const rightGamepadButtonStates: boolean[] = [];

// Helper function to load GLTF and create both rendering VAO and collision mesh
async function createGltfObjectWithCollision(
  modelPath: string,
  position: Vector3,
  rotation: Matrix4,
  scale: Vector3 | number
): Promise<{ vao: VertexArray; collisionMesh: Trimesh; position: Vector3; rotation: Matrix4; scale: Vector3 | number }> {
  // Load the GLTF model
...
async function initialize() {
  ...
  const platform1 = await createGltfObjectWithCollision(
    '../../models/platform.gltf',
    new Vector3(terrainCenterX, platformCenterY, terrainCenterZ),
    Matrix4.rotateX(0),
    15
  );
  ...
  const platform2 = await createGltfObjectWithCollision(
    '../../models/platform.gltf',
    new Vector3(terrainCenterX + 200, platformCenterY + 100, terrainCenterZ + 160),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 3
  const platform3 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 200, platformCenterY + 200, terrainCenterZ - 150),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 4
  const platform4 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 180, platformCenterY + 320, terrainCenterZ - 180),
    Matrix4.rotateX(0),
  ...
  // Platform 5
  const platform5 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 150, platformCenterY + 470, terrainCenterZ + 190),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 6
  const platform6 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 210, platformCenterY + 620, terrainCenterZ + 170),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 7
  const platform7 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 190, platformCenterY + 770, terrainCenterZ - 160),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 8
  const platform8 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 170, platformCenterY + 920, terrainCenterZ - 200),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 9
  const platform9 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 180, platformCenterY + 1070, terrainCenterZ + 180),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 10
  const platform10 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 190, platformCenterY + 1220, terrainCenterZ + 150),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 11
  const platform11 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 170, platformCenterY + 1370, terrainCenterZ - 170),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 12
  const platform12 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 160, platformCenterY + 1520, terrainCenterZ - 190),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 13
  const platform13 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 190, platformCenterY + 1670, terrainCenterZ + 160),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 14
  const platform14 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 180, platformCenterY + 1820, terrainCenterZ - 180),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 15
  const platform15 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 200, platformCenterY + 1970, terrainCenterZ - 150),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 16
  const platform16 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 170, platformCenterY + 2120, terrainCenterZ + 190),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 17
  const platform17 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 185, platformCenterY + 2270, terrainCenterZ + 175),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 18
  const platform18 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 195, platformCenterY + 2420, terrainCenterZ - 165),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 19
  const platform19 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 175, platformCenterY + 2570, terrainCenterZ - 185),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 20
  const platform20 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 160, platformCenterY + 2720, terrainCenterZ + 180),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 21
  const platform21 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 190, platformCenterY + 2870, terrainCenterZ + 170),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 22
  const platform22 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 185, platformCenterY + 3020, terrainCenterZ - 175),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 23
  const platform23 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 165, platformCenterY + 3170, terrainCenterZ - 195),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 24
  const platform24 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 200, platformCenterY + 3320, terrainCenterZ + 185),
    Matrix4.rotateX(0),
    15
  );
  ...
  // Platform 25
  const platform25 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 180, platformCenterY + 3470, terrainCenterZ + 160),
    Matrix4.rotateX(0),
    15
  );
  ...
  const platform26 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 190, platformCenterY + 3620, terrainCenterZ - 170),
    Matrix4.rotateX(0),
    new Vector3(6, 15, 15)
  );
  ...
  const platform27 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 160, platformCenterY + 3770, terrainCenterZ - 190),
    Matrix4.rotateX(0),
  ...
  const platform28 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 175, platformCenterY + 3920, terrainCenterZ + 185),
    Matrix4.rotateX(0),
    new Vector3(15, 15, 6)
  );
  ...
  const platform29 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 185, platformCenterY + 4070, terrainCenterZ + 170),
    Matrix4.rotateX(0),
  ...
  const platform30 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 195, platformCenterY + 4220, terrainCenterZ - 180),
    Matrix4.rotateX(0),
  ...
  const platform31 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 170, platformCenterY + 4370, terrainCenterZ - 195),
    Matrix4.rotateY(45),
    new Vector3(25, 15, 6)
  );
  ...
  const platform32 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 180, platformCenterY + 4520, terrainCenterZ + 175),
    Matrix4.rotateX(0),
  ...
  const platform33 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 190, platformCenterY + 4670, terrainCenterZ + 165),
    Matrix4.rotateX(0),
    new Vector3(7, 15, 15)
  );
  ...
  const platform34 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 185, platformCenterY + 4820, terrainCenterZ - 175),
    Matrix4.rotateX(0),
  ...
  const platform35 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 165, platformCenterY + 4970, terrainCenterZ - 185),
    Matrix4.rotateX(0),
    new Vector3(20, 15, 15)
  );
  ...
  const platform36 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 200, platformCenterY + 5120, terrainCenterZ + 190),
    Matrix4.rotateX(0),
  ...
  const platform37 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 175, platformCenterY + 5270, terrainCenterZ + 180),
    Matrix4.rotateX(0),
    new Vector3(6, 15, 20)
  );
  ...
  const platform38 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 190, platformCenterY + 5420, terrainCenterZ - 170),
    Matrix4.rotateX(0),
  ...
  const platform39 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 180, platformCenterY + 5570, terrainCenterZ - 160),
    Matrix4.rotateY(30),
    new Vector3(5, 15, 20)
  );
  ...
  // Platform 40
  const platform40 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 0, platformCenterY + 5720, terrainCenterZ + 0),
    Matrix4.rotateX(0),
  ...
  // Platform 41
  const platform41 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 280, platformCenterY + 5920, terrainCenterZ + 250),
    Matrix4.rotateX(0),
    new Vector3(15, 15, 15)
  );
  ...
  // Platform 42
  const platform42 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 290, platformCenterY + 6020, terrainCenterZ - 260),
    Matrix4.rotateX(0),
    new Vector3(13, 15, 13)
  );
  ...
  // Platform 43
  const platform43 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 270, platformCenterY + 6320, terrainCenterZ - 280),
    Matrix4.rotateX(0),
    new Vector3(35, 15, 35)
  );
  ...
  // Platform 44
  const platform44 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 300, platformCenterY + 6520, terrainCenterZ + 270),
    Matrix4.rotateX(0),
    new Vector3(27, 15, 27)
  );
  ...
  // Platform 45
  const platform45 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX - 290, platformCenterY + 6720, terrainCenterZ + 260),
    Matrix4.rotateX(0),
    new Vector3(27, 15, 37)
  );
  ...
  const platform46 = await createGltfObjectWithCollision(
    platformLink,
    new Vector3(terrainCenterX + 0, platformCenterY + 6920, terrainCenterZ + 0),
    Matrix4.rotateX(0),
...
function updateGameState(elapsed: number) {
...
function handleWin(playerNumber: number) {
  gameWon = true;
...
function clampCameraToTerrain(camera: FirstPersonCamera) {
...
function updateCameraWithKeyboard(camera: FirstPersonCamera, elapsed: number) {
...
function checkGamepadKonamiCode(
  gamepad: Gamepad,
  sequence: string[],
  lastInputTime: number,
  buttonStates: boolean[]
): { sequence: string[], lastInputTime: number, activated: boolean } {
  const currentTime = Date.now();
  let activated = false;
  ...
  const buttonMapping: { [button: number]: string } = {
    12: 'arrowup',
    13: 'arrowdown',
    14: 'arrowleft',
    15: 'arrowright',
...
function updateCameraWithGamepad(camera: FirstPersonCamera, gamepad: Gamepad, elapsed: number) {
  // Determine which player this is
  const animController = camera === cameraLeft ? leftPlayerAnimController : rightPlayerAnimController;
  ...
  if (gameState === GameState.PLAYING) {
    if (isLeftPlayer) {
      const result = checkGamepadKonamiCode(
        gamepad,
        leftGamepadKonamiSequence,
        lastLeftGamepadKonamiInputTime,
        leftGamepadButtonStates
      );
      ...
      const result = checkGamepadKonamiCode(
        gamepad,
        rightGamepadKonamiSequence,
        lastRightGamepadKonamiInputTime,
        rightGamepadButtonStates
      );
...
function renderScene(camera: FirstPersonCamera, clipFromEye: Matrix4) {
  ...
  for (let i = 0; i < towerLightPositions.length; i++) {
    ...
    const worldFromTowerLight = Matrix4.translate(
      towerLightPos.x,
      towerLightPos.y + towerLightBobOffset, // Add bobbing offset
      towerLightPos.z
    ).multiplyMatrix(Matrix4.scale(scaleFlicker, scaleFlicker * 1.2, scaleFlicker)); // Elongated vertically like flame
  ...
  if (diamondVisible) {
    ...
    const worldFromDiamond = Matrix4.translate(
      diamondPosition.x,
      diamondPosition.y,
      diamondPosition.z
    ).multiplyMatrix(Matrix4.rotateY(diamondRotation))
...
function renderSkybox(camera: FirstPersonCamera, clipFromEye: Matrix4) {
...
function renderPlatform(
  ...
  const worldFromPlatform = Matrix4.translate(
    platform.position.x,
    platform.position.y,
    platform.position.z
  ).multiplyMatrix(platform.rotation)
...
function renderPlayer(
  ...
  const horizontalForward = new Vector3(
    playerCamera.forward.x,
    0,  // Zero out Y component to ignore pitch
    playerCamera.forward.z
  ).normalize();
  ...
  const modelPosition = new Vector3(
    playerCamera.from.x + horizontalForward.x * offsetForward + cameraRight.x * offsetRight,
    playerCamera.from.y + offsetUp,  // Use camera's Y position with offset
  ...
  if (isGodMode) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const worldFromGlow = Matrix4.translate(
        modelPosition.x,
        modelPosition.y,
        modelPosition.z
      ).multiplyMatrix(orientation)
...
function renderTower(
  ...
  rotation: Matrix4,
  scale: number,
  color: Vector3 = new Vector3(0.6, 0.6, 0.7)
) {
  const worldFromTower = Matrix4.translate(
    position.x,
    position.y,
    position.z
...
function renderWall(
  ...
  const worldFromWall = Matrix4.translate(
    position.x,
    position.y,
    position.z
  ).multiplyMatrix(rotation)
...
function checkPlayerHit() {
  ...
  const leftHorizontalForward = new Vector3(
    cameraLeft.forward.x,
    0,
    cameraLeft.forward.z
  ).normalize();
  const leftPlayerPos = new Vector3(
    cameraLeft.from.x + leftHorizontalForward.x * offsetForward,
    cameraLeft.from.y + offsetUp,
    cameraLeft.from.z + leftHorizontalForward.z * offsetForward
  );

  // Right player position
  const rightHorizontalForward = new Vector3(
    cameraRight.forward.x,
    0,
    cameraRight.forward.z
  ).normalize();
  const rightPlayerPos = new Vector3(
    cameraRight.from.x + rightHorizontalForward.x * offsetForward,
    cameraRight.from.y + offsetUp,
    cameraRight.from.z + rightHorizontalForward.z * offsetForward
  );
...
function respawnPlayer(isLeftPlayer: boolean) {
  if (isLeftPlayer) {
...
function updateCompass(camera: FirstPersonCamera, compassDiv: HTMLDivElement) {
...
function updateAltimeter(camera: FirstPersonCamera, altimeterDiv: HTMLDivElement) {

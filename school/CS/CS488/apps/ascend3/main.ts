...
function createGltfObjectWithCollision(
  gltfMesh: any,
  position: Vector3,
  rotation: Matrix4,
  scale: number
): { vao: VertexArray; collisionMesh: Trimesh; position: Vector3; rotation: Matrix4; scale: number } {
  ...
  const transformedPositions = positions.map(pos => {
    ...
    return new Vector3(
      rotated.x * scale + position.x,
      rotated.y * scale + position.y,
      rotated.z * scale + position.z
    );
  });
...
async function initialize() {
  ...
  const texBuf = (mesh as any).texCoords;
  if (texBuf) {
  ...
  const platform1 = createGltfObjectWithCollision(
    platformModelMesh,
  ...
  const platform2 = createGltfObjectWithCollision(
    platformModelMesh,
...
function updateCameraWithGamepad(camera: FirstPersonCamera, gamepad: Gamepad, elapsed: number, side: 'left' | 'right') {
...
function renderPlayer(
  ...
  const modelPosition = new Vector3(
    playerCamera.from.x + forward.x * offsetForward + right.x * offsetRight + cameraUp.x * offsetDown,
    playerCamera.from.y + forward.y * offsetForward + right.y * offsetRight + cameraUp.y * offsetDown,
    playerCamera.from.z + forward.z * offsetForward + right.z * offsetRight + cameraUp.z * offsetDown
  );

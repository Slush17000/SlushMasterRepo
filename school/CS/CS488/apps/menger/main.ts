...
let shaderProgram2D: ShaderProgram;
let shaderProgram3D: ShaderProgram;
let vao: VertexArray;
let currentLevel: number = 3;
let is3DMode: boolean = false;
 ...
 */
function generateMenger2D(level: number): Float32Array {
    const squares: number[] = [];
    
    // Helper function to add a square (two triangles)
    function addSquare(x: number, y: number, size: number) {
    ...
    function subdivide(x: number, y: number, size: number, depth: number) {
        if (depth === 0) {
 ...
 */
function generateMenger3D(level: number): { positions: Float32Array, normals: Float32Array, vertexCount: number } {
    const vertices: number[] = [];
    const normals: number[] = [];
    
    // Helper function to add a cube face (two triangles)
    function addFace(v0: Vector3, v1: Vector3, v2: Vector3, v3: Vector3) {
    ...
    function addCube(x: number, y: number, z: number, size: number) {
        const halfSize = size / 2;
    ...
    function subdivide(x: number, y: number, z: number, size: number, depth: number) {
        if (depth === 0) {

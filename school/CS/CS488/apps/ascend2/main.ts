...
import { padToPot } from 'lib/web-utilities.js';
...
async function initialize() {
  ...
  if ((mesh as any).texCoords) {
    attributes.addAttribute('texPosition', mesh.vertexCount, 2, (mesh as any).texCoords);
  }
...
function renderPlayer(
  ...
  const worldFromPlayer = Matrix4.translate(
    ...
    modelPosition.y,
    modelPosition.z
  ).multiplyMatrix(Matrix4.rotateY(yawDegrees - 90))

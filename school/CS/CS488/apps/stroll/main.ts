...
let heightField: Field2;
let factors: Vector3;
let currentHeightMap: string;

async function loadHeightMap(heightMapPath: string) {
...
function updateCamera(elapsed: number) {
  ...
  if (keys['w']) camera.advance(elapsed * moveSpeed);
  if (keys['s']) camera.advance(-elapsed * moveSpeed);
  if (keys['a']) camera.strafe(-elapsed * moveSpeed);
  if (keys['d']) camera.strafe(elapsed * moveSpeed);

  currentPitch = Math.asin(camera.forward.y) * 180 / Math.PI;
  if (keys['arrowdown'] && currentPitch > -85) camera.pitch(-elapsed * lookSpeed);
  if (keys['arrowup'] && currentPitch < 85) camera.pitch(elapsed * lookSpeed);

  // QE for yaw
  if (keys['arrowleft']) camera.yaw(elapsed * lookSpeed);
  if (keys['arrowright']) camera.yaw(-elapsed * lookSpeed);

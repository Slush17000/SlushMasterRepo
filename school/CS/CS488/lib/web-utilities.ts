...
export function generateRgbaImage(width: number, height: number) {
...
export function generateGrayscaleImage(width: number, height: number) {
...
export function generateCheckerboardImage(width: number, height: number) {
  const n = width * height * 4;
  const pixels = new Uint8ClampedArray(n);
...
export function generateAlphaGradientImage(width: number, height: number) {
  const n = width * height * 4;
  const pixels = new Uint8ClampedArray(n);
  for (let r = 0; r < height; ++r) {
    for (let c = 0; c < width; ++c) {
...
export function padToPot(image: HTMLImageElement): ImageData {
...
export function powerOfTwoCeiling(x: number): number {
  return Math.pow(2, Math.ceil(Math.log2(x)));
}

export function generateCrateTexture(width: number, height: number): Uint8ClampedArray {
  const n = width * height * 4;
  const pixels = new Uint8ClampedArray(n);

...
export function intersectRaySphere(rayStart: Vector3, rayDirection: Vector3, sphereCenter: Vector3, sphereRadius: number): Vector3[] {
...
export function intersectRayPyramid(rayStart: Vector3, rayDirection: Vector3, baseSize: number, height: number): Vector3[] {
        ...
        if (Math.abs(denom) > 0.0001) {
            ...
            if (t >= 0) {
                ...
                if (u >= 0 && v >= 0 && u + v <= 1) {
                    ...
                    const isUnique = intersections.every(existing =>
                        existing.subtract(p).magnitude > 0.0001
                    );
                    if (isUnique) {
                        intersections.push(p);
                    }
...
export function intersectRayCone(rayStart: Vector3, rayDirection: Vector3, topRadius: number, bottomRadius: number, height: number): Vector3[] {
    const intersections: Vector3[] = [];
    ...
    if (topRadius > 0.0001 && Math.abs(rayDirection.y) > 0.0001) {
        ...
        if (t >= 0) {
            ...
            if (distFromCenter <= topRadius + 0.001) {
                const isUnique = intersections.every(existing =>
                    existing.subtract(p).magnitude > 0.001
                );
                if (isUnique) {
                    intersections.push(p);
                }
...
export function intersectRayTetrahedron(rayStart: Vector3, rayDirection: Vector3, size: number): Vector3[] {
    const intersections: Vector3[] = [];
...
export function intersectRayOctahedron(rayStart: Vector3, rayDirection: Vector3, size: number): Vector3[] {
    const intersections: Vector3[] = [];
...
export function intersectRayDodecahedron(rayStart: Vector3, rayDirection: Vector3, size: number): Vector3[] {
    const intersections: Vector3[] = [];
...
export function intersectRayIcosahedron(rayStart: Vector3, rayDirection: Vector3, size: number): Vector3[] {
    const intersections: Vector3[] = [];

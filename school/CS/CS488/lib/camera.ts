...
export class FirstPersonCamera {
    ...
    field: Field2 | null;
    offset: number;
    factors: Vector3 | null;

    // Mesh collision support
    meshes: Trimesh[];
    meshInteractionEnabled: boolean;

    // Momentum system
    momentumForward: number;
    momentumStrafe: number;
    acceleration: number;    // How quickly momentum builds up (0-1)
    deceleration: number;    // How quickly momentum slows down (0-1)

    // Knockback system
    knockbackVelocity: Vector3;
    knockbackDecay: number; // How quickly knockback velocity decreases

    // God mode
    godMode: boolean;

    constructor(from: Vector3, to: Vector3, worldUp: Vector3, field: Field2 | null = null, offset: number = 0, factors: Vector3 | null = null) {
    ...
    updateMomentum(inputForward: number, inputStrafe: number) {
    ...
    applyMomentumMovement(speed: number) {
    ...
    applyKnockback(direction: Vector3, strength: number) {
    ...
    addMesh(mesh: Trimesh) {
        this.meshes.push(mesh);
    }

    removeMesh(mesh: Trimesh) {
    ...
    private intersectRayBox(rayStart: Vector3, rayDirection: Vector3, boxMin: Vector3, boxMax: Vector3): Vector3[] {
     ...
     */
    private getMeshCeilingAt(mesh: Trimesh, x: number, z: number): number | null {
     ...
     */
    private getMeshHeightAt(mesh: Trimesh, x: number, z: number): number | null {
        // Quick bounding box check first
        if (x < mesh.min.x || x > mesh.max.x || z < mesh.min.z || z > mesh.max.z) {
            return null;
        }

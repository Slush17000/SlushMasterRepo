// TAKEN DIRECTLY FROM CHRONO DEMO FILES
// MODIFIED FOR THREAD COUNT INPUT AND TERRAIN GRANULAR VS FLAT INPUT
// MODIFIED TOTAL SIMULATION TIME FROM 7 TO 5 SECONDS
//=============================================================================
...
int main(int argc, char* argv[]) {
    ...
    // Take terrain type input
    std::string terrain_type = "granular";
    if (argc > 2) {
        terrain_type = argv[2];
    }
    ...
    ChTerrain* terrain = nullptr;
    uint actual_num_particles = 0;

    if (terrain_type == "granular") {
        auto gterrain = new GranularTerrain(sys);
        auto mat = std::static_pointer_cast<ChContactMaterialNSC>(gterrain->GetContactMaterial());
        mat->SetFriction((float)mu_g);
        mat->SetCohesion((float)coh_g);
        gterrain->SetContactMaterial(mat);
        gterrain->SetCollisionEnvelope(envelope / 5);
        if (rough) {
            int nx = (int)std::round((2 * hdimX) / (4 * r_g));
            int ny = (int)std::round((2 * hdimY) / (4 * r_g));
            gterrain->EnableRoughSurface(nx, ny);
        }
        gterrain->EnableVisualization(true);
        gterrain->EnableVerbose(true);

        gterrain->Initialize(ChVector3d(0, 0, 0), 2 * hdimX, 2 * hdimY, num_layers, r_g, rho_g);
        actual_num_particles = gterrain->GetNumParticles();
        terrain = gterrain;

        std::cout << "Using granular terrain with " << gterrain->GetNumParticles() << " particles.\n";

    } else if (terrain_type == "rigid") {
        auto rterrain = new RigidTerrain(sys);
        auto patch_mat = chrono_types::make_shared<ChContactMaterialNSC>();
        patch_mat->SetFriction(0.9f);
        patch_mat->SetRestitution(0.01f);
        double patch_len = 10;
        rterrain->AddPatch(patch_mat, ChCoordsys<>(ChVector3d(0, 0, 0), QUNIT), 2 * hdimX, 2 * hdimY);
        rterrain->Initialize();
        terrain = rterrain;

        std::cout << "Using rigid terrain.\n";
    } else {
        std::cerr << "Unknown terrain type: " << terrain_type << ". Use 'granular' or 'rigid'.\n";
        return 1;
    }
    ...
    while (true) {
        ...
        if (!hmmwv && time > time_create_vehicle) {
            ...
            if (terrain_type == "granular" && moving_patch) {
                static_cast<GranularTerrain*>(terrain)->EnableMovingPatch(hmmwv->GetChassisBody(), buffer_distance, shift_distance);
            }
